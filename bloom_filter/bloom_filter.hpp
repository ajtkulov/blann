...
public:
   ...
   void serializeVectorToFile(const std::string& filename) {
      std::ofstream file(filename, std::ios::binary);
      if (file) {
         size_t size = bit_table_.size();
         file.write(reinterpret_cast<const char*>(&size), sizeof(size_t));
         file.write(reinterpret_cast<const char*>(bit_table_.data()), size * sizeof(unsigned char));
         file.close();
      } else {
